...
def run_backtest(
    ...
    for i in range(len(df)):
        ...
        if current_trade:
            exit_price = None
            status = None
            trade_type = current_trade['fvg_type'] # This 'fvg_type' is from current_trade, which was set correctly using fvg['type']
            ...
            for j in range(processed_fvg_idx, len(potential_fvgs)):
                ...
                if actual_entry_price_for_trade is not None:
                    ...
                    if distance_to_sl <= 1e-9: # Use a small epsilon for float comparison
                        logger.warning(f"Trade on FVG at {fvg['time']} for {fvg['type']} skipped: distance to SL is effectively zero ({distance_to_sl:.4f}).")
                    ...
                    current_trade = {
                        "fvg_type": fvg['type'], # Key fix: was fvg_type
                        "entry_price": actual_entry_price_for_trade,
                        "sl_price": sl_price_fvg,
                        "tp_price": fvg['tp_price'], # TP from FVG definition
                        "entry_time": candle_time,
                        "fvg_details": fvg,
                        "position_size": position_size,
                        "amount_risked": risk_amount_for_new_trade
                    }
                    processed_fvg_idx = j + 1
                    logger.debug(f"Trade opened: {fvg['type']} at {actual_entry_price_for_trade:.2f}. Size: {position_size:.4f}. SL: {sl_price_fvg:.2f}. Risked: {risk_amount_for_new_trade:.2f}") # Key fix: was fvg_type
                    break
