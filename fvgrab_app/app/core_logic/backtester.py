...
def run_backtest(
    ...
    if not all(col in ohlcv_df.columns for col in required_cols):
        # This error should be caught by the API layer if parameters are incorrect.
    ...
    potential_fvgs.sort(key=lambda x: pd.to_datetime(x['trigger_candle_time']))
    ...
    for i in range(len(df)):
        ...
        if current_trade:
            exit_price = None; status = None
            trade_type = current_trade['fvg_type']; sl_target = current_trade['sl_price']
            tp_target = current_trade['tp_price']; entry_price = current_trade['entry_price']
            position_size = current_trade['position_size']; amount_risked = current_trade['amount_risked']
            ...
            if exit_price is not None and status is not None:
                ...
                simulated_trades.append({
                    ...
                    "pnl_percent": pnl_percent_on_risked_capital, "commission_paid": total_commission_for_trade,
        ...
        if current_trade is None:
            if current_balance <= 0: logger.warning(f"Balance is {current_balance:.2f} at {candle_time}, cannot open new trades."); continue
            risk_amount_for_new_trade = current_balance * (risk_per_trade_percent / 100.0)
            if risk_amount_for_new_trade <= 0: logger.info(f"Calculated risk amount is {risk_amount_for_new_trade:.2f} at {candle_time}, skipping."); continue
            for j in range(processed_fvg_idx, len(potential_fvgs)):
                ...
                entry_price_target = fvg['entry_price']; sl_price_fvg = fvg['sl_price']
                actual_entry_price_for_trade = None
                if candle_low <= entry_price_target <= candle_high: actual_entry_price_for_trade = entry_price_target
                if actual_entry_price_for_trade is not None:
                    distance_to_sl = abs(actual_entry_price_for_trade - sl_price_fvg)
                    if distance_to_sl <= 1e-9: # Using a small epsilon for float comparison
                        logger.warning(f"Trade on FVG at {fvg['time']} for {fvg['fvg_type']} skipped: SL distance zero ({distance_to_sl:.4f}).")
                        if j == processed_fvg_idx: processed_fvg_idx = j + 1
                        continue
                    position_size = risk_amount_for_new_trade / distance_to_sl
                    current_trade = {"fvg_type": fvg['fvg_type'], "entry_price": actual_entry_price_for_trade,
                                     "sl_price": sl_price_fvg, "tp_price": fvg['tp_price'], "entry_time": candle_time,
                                     "fvg_details": fvg, "position_size": position_size, "amount_risked": risk_amount_for_new_trade}
    ...
    metrics_results = calculate_performance_metrics(trades=simulated_trades, initial_balance=initial_balance, risk_free_rate_annual=risk_free_rate_annual)
    return {"trades": simulated_trades, "performance_metrics": metrics_results}

def calculate_performance_metrics(trades: List[Dict[str, Any]], initial_balance: float, risk_free_rate_annual: float = 0.0) -> Dict[str, Any]:
    # TODO: Implement a more rigorous Sharpe Ratio (e.g., annualized, using daily portfolio returns, proper scaling of risk-free rate).
    # TODO: Calculate Buy and Hold return for the period using ohlcv_df (would need ohlcv_df as input here or just its start/end prices).
    # TODO: Consider generating data suitable for an equity curve plot (e.g., list of [timestamp, equity]).
    # TODO: Add other metrics like Sortino Ratio, Calmar Ratio, longest winning/losing streak, etc.
    metrics = {"total_trades": 0, "winning_trades": 0, "losing_trades": 0, "neutral_trades": 0,
               "win_rate_percent": 0.0, "loss_rate_percent": 0.0, "profit_factor": 0.0,
               "total_net_pnl_absolute": 0.0, "average_pnl_per_trade_absolute": 0.0,
               "average_profit_per_winning_trade_absolute": 0.0, "average_loss_per_losing_trade_absolute": 0.0,
               "average_holding_time_hours": 0.0, "sharpe_ratio_simplified": 0.0,
               "max_drawdown_percent": 0.0, "final_equity": initial_balance, "peak_equity": initial_balance,
               "buy_and_hold_return_percent": 0.0, "total_commission_paid": 0.0}
    ...
    for trade in trades:
        ...
        entry_time = pd.to_datetime(trade['entry_time']); exit_time = pd.to_datetime(trade['exit_time'])
        if exit_time > entry_time: total_holding_time_seconds += (exit_time - entry_time).total_seconds()
    metrics.update({"total_trades": total_trades, "winning_trades": winning_trades, "losing_trades": losing_trades,
    ...
    if total_trades > 0:
        ...
        if total_holding_time_seconds > 0: metrics["average_holding_time_hours"] = (total_holding_time_seconds / total_trades) / 3600.0
    ...
    if len(pnl_percentages_for_sharpe) > 1:
        mean_return = np.mean(pnl_percentages_for_sharpe); std_return = np.std(pnl_percentages_for_sharpe)
        if std_return > 1e-9: metrics["sharpe_ratio_simplified"] = mean_return / std_return # Avoid division by zero if all returns are same
